...
export default async function handler(request: VercelRequestLike, response: VercelResponseLike) {
  ...
  const isPushAction = request.method === "POST" && ["push_subscribe", "push_unsubscribe", "push_test"].includes(String(body.action));
  if (isPushAction && !isOperatorAuthenticated(request)) {
  ...
  if (isPushAction) {
    try {
      ...
      if (body.action === "push_subscribe") await saveOperatorSubscription(repository, body.subscription);
      else if (body.action === "push_unsubscribe") await removeOperatorSubscription(repository, body.subscription);
      else await sendOperatorNotification(repository, "test", "test");
      response.status(200).json({ ok: true });
    } catch (error) {
      response.status(400).json({ ok: false, error: error instanceof Error ? error.message : "Push request failed" });
    }
    return;
  }
  ...
  if (request.method === "POST" && body.action === "scan_due_site") {
    try {
      assertSameOrigin(request);
      assertOperatorCsrf(request, body);
      const [site] = await repository.listSitesDueContentLearning(1);
      if (!site) {
        response.status(200).json({ ok: true, status: "idle" });
        return;
      }
      const learning = await processSiteContentLearning(repository, site, 4);
      if (learning.status === "completed") {
      ...
      response.status(200).json({ ok: true, status: learning.status, processed: learning.processedCount });
    } catch (error) {
      console.warn("[operator-site-learning] failed", {
        errorName: error instanceof Error ? error.name : "Error",
        errorMessage: error instanceof Error ? error.message : String(error),
      });
      response.status(500).json({ ok: false, error: "Website learning could not continue" });
    }
    return;
  }

  if (request.method === "POST" && body.action === "run_due_research") {
    try {
      assertSameOrigin(request);
      assertOperatorCsrf(request, body);
      const [site] = await repository.listDueOperatorManagedSites(1);
      if (!site) {
        response.status(200).json({ ok: true, status: "idle" });
        return;
      }
      const result = await createScheduledOperatorContentJob(repository, site);
      response.status(200).json({ ok: true, ...result });
    } catch (error) {
      console.warn("[operator-due-research] failed", {
        errorName: error instanceof Error ? error.name : "Error",
        errorMessage: error instanceof Error ? error.message : String(error),
      });
      response.status(500).json({ ok: false, error: "Scheduled research could not start" });
    }
    return;
  }

  const [jobs, connections, learningSites] = await Promise.all([
    repository.listOperatorContentJobs(),
    repository.listPendingSiteConnections(),
    repository.listSitesDueContentLearning(1),
  ]);
  ...
  const connectionRequests = connections.length ? `<section class="priority"><h2>Connection requests · ${connections.length}</h2><div class="priority-list">${connections.map((connection) => `<article class="priority-item"><div><strong>${html(connection.business_name || "New customer")}</strong><p class="timing">${html(connection.website_url)} · Requested ${html(timeAgo(connection.requested_at))}</p></div><form method="post"><input type="hidden" name="csrf" value="${csrf}"><input type="hidden" name="connection_id" value="${html(connection.id)}"><button class="button secondary" type="submit" name="action" value="reject_connection">Reject</button> <button class="button" type="submit" name="action" value="approve_connection">Approve</button></form></article>`).join("")}</div></section>` : "";
  const learningPanel = learningSites.length
    ? `<section class="priority" id="site-learning"><h2>Learning customer website</h2><p class="sub" id="site-learning-status">Securely reading the next group of public pages and posts…</p></section><script nonce="__CSP_NONCE__">addEventListener("load",async()=>{const status=document.getElementById("site-learning-status");try{const response=await fetch("/api/operator",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify({action:"scan_due_site",csrf:${JSON.stringify(csrf)}})});const result=await response.json();if(!response.ok)throw new Error(result.error||"Website learning failed");status.textContent=(result.status==="completed"?"Website learning completed · ":"Website learning in progress · ")+(result.processed||0)+" public items learned";if(result.status==="completed"||result.status==="learning")setTimeout(()=>location.reload(),800)}catch(error){status.textContent=error.message||"Website learning could not continue"}});</script>`
    : "";
  const dueResearchKick = `<script nonce="__CSP_NONCE__">addEventListener("load",async()=>{try{const response=await fetch("/api/operator",{method:"POST",credentials:"same-origin",headers:{"content-type":"application/json"},body:JSON.stringify({action:"run_due_research",csrf:${JSON.stringify(csrf)}})});const result=await response.json();if(response.ok&&result.status==="brief_ready")location.reload()}catch{}});</script>`;
  queues = learningPanel + dueResearchKick + connectionRequests + priorityInbox + queues;
