...
export default async function handler(request: VercelRequestLike, response: VercelResponseLike) {
  ...
  const queue = (title: string, items: Record<string, unknown>[], omitWhenEmpty = false) => {
    if (!items.length && omitWhenEmpty) return "";
    return `<section class="queue"><div class="queue-head"><h2>${html(title)}</h2><span class="count">${items.length}</span></div>${items.length ? items.map(renderJob).join("") : '<div class="empty">Nothing here right now.</div>'}</section>`;
  };
  ...
  if (view === "customer") {
    ...
    queues = [...customers.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([customer, items]) => queue(customer, items)).join("");
  } else if (view === "status") {
    queues = sections.map(({ key, title }) => queue(title, visibleJobs.filter((job) => group(job.status) === key), key === "completed")).join("");
  } else {
    ...
    queues = actionSections.map(({ key, title }) => queue(title, visibleJobs.filter((job) => action(job.status) === key), key === "completed")).join("");
  }
