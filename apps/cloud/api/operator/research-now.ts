  ...
  if (request.method === "POST") {
    try {
      ...
      if (String((result as Record<string, unknown>).status ?? "") === "deferred") {
        ...
        return sendHtml(response, 409, `<main class="shell"><div class="top"><h1>Research not started</h1><a href="/api/operator/research-now">Back</a></div><p class="notice warning">${reason}</p><p>No rule was bypassed. The existing research gates remain in force.</p></main>`);
      }
      response.setHeader?.("location", "/api/operator?view=action");
      response.status(303).send?.("");
      return;
    } catch (error) {
      return sendHtml(response, 400, `<main class="shell"><div class="top"><h1>Research not started</h1><a href="/api/operator/research-now">Back</a></div><p class="notice warning">${html(error instanceof Error ? error.message : error)}</p></main>`);
    }
  }
